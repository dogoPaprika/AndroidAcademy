...
val reviewSummaries = listOf(
    ReviewSummary(3.0, 1, 46067),
...
val reviews = LongSparseArray<List<Review>>(poiList.size).also { map ->
    poiList.forEachIndexed { index, it ->
        map.put(
            it.id,
            randomReviewList(
                it.id,
                reviewSummaries[index].numberOfReviews,
                reviewSummaries[index].averageRating
            )
        )
    }
}
